(() => {
  ...
  const photoFor = (query,title) => {const collection=window.TRIP_PLACE_PHOTOS;const id=collection?.titles?.[title]||collection?.places?.[query];return id?collection.assets[id]:null;};
  ...
  const placePhoto = (query, title, compact=false) => {const photo=photoFor(query,title);if(!photo)return '';const caption=photoCaption(photo,title);return `<figure class="place-photo${compact?' compact':''}"><img src="${e(photo.src)}" alt="${e(caption)}"${photo.position?` style="object-position:${e(photo.position)}"`:''} loading="lazy" decoding="async"><figcaption><span>${e(caption)}</span><a href="${e(photo.source)}" target="_blank" rel="noopener noreferrer" aria-label="${e(title)}の写真情報">写真情報 ↗</a></figcaption></figure>`;};
