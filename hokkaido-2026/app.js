(() => {
  ...
  const photoFor = query => {const collection=window.TRIP_PLACE_PHOTOS;const id=collection?.places?.[query];return id?collection.assets[id]:null;};
  const photoCaption = (photo,title) => title.startsWith(photo.caption)||photo.caption.startsWith(title)?photo.caption:`${photo.caption}（周辺の風景）`;
  const placePhoto = (query, title, compact=false) => {const photo=photoFor(query);if(!photo)return '';const caption=photoCaption(photo,title);return `<figure class="place-photo${compact?' compact':''}"><img src="${e(photo.src)}" alt="${e(caption)}" loading="lazy" decoding="async"><figcaption><span>${e(caption)}</span><a href="${e(photo.source)}" target="_blank" rel="noopener noreferrer" aria-label="${e(title)}の写真情報">写真情報 ↗</a></figcaption></figure>`;};
  ...
  function eventCard(event,index,events) {
    ...
    return `<article class="stop${event.fixed?' fixed':''}"><div class="time-col">${e(event.time)}<small>${e(event.kind)}</small></div><div class="stop-body">${placePhoto(event.query,event.title)}<div class="stop-meta">${e(event.category)}</div><h3>${e(event.title)}</h3><p>${e(event.text)}</p>${event.address?`<p class="address">${e(event.address)}</p>`:''}<div class="stop-actions">${actions.join('')}</div></div>${event.leg?`<div class="leg">${icon(event.leg.includes('徒歩')?'pin':'car')}${e(event.leg)}</div>`:''}</article>`;
  }
  ...
  function render() {
    ...
    if(state.mode==='plan'||state.mode==='explore')window.TripMap.mount(trip.days[state.day-1],trip.spots.filter(s=>s.day===state.day&&(state.mode==='plan'||state.filter==='all'||(state.filter==='enRoute'?s.enRoute:state.filter==='indoors'?s.indoors:s.type===state.filter))));
    updateConnection();
  }
  function parseRoute() {
